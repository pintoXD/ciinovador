...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............March 13, 2025 at 18:57:13                     //
//                                                                           //
//  PROJECT NAME..............work                                           //
//                                                                           //
//  TESTMODE..................FULLSCAN                                       //
...
// Command Line: write_vectors -WORKDIR /home/cinovador/Documents/course_files/physical_synthesis/sequential_ufc_ufcg_ufsm/sequential_dft_atpg/backend/synthesis/work -TESTMODE FULLSCAN -INEXPERIMENT logic -language verilog -outputfilename sequential_test_results -scanformat serial

  `timescale 1 ns / 1 ps

  module work_FULLSCAN_logic ;
...
//***************************************************************************//
  ...
  sequential
    sequential_inst (
      .A        ( part_PIs[1] ),      // pinName = A; testOffset = 0.000000;  scanOffset = 0.000000;  
      .B        ( part_PIs[2] ),      // pinName = B; testOffset = 0.000000;  scanOffset = 0.000000;  
      .C        ( part_PIs[3] ),      // pinName = C; testOffset = 0.000000;  scanOffset = 0.000000;  
      ...
      .Q        ({part_POs[2]  ,      // pinName = Q[1]; 
                  part_POs[1]}),      // pinName = Q[0]; 
      .SE       ( part_PIs[4] ),      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",sequential_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",sequential_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",sequential_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
      ...
      if ( $test$plusargs ( "vcd" ) )  begin 
        $dumpfile("out.vcd"); 
        $dumpvars(0,work_FULLSCAN_logic ); 
      end  
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  sequential   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      part_PIs [1] = stim_PIs [1];      // pinName = A; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [2] = stim_PIs [2];      // pinName = B; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [3] = stim_PIs [3];      // pinName = C; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [4] = stim_PIs [4];      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     ...
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 3; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sequential", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
...
//***************************************************************************//
  ...
  task Scan_Preconditioning_Sequence_TM_1_SEQ_1_SOP_1; 
    begin 
      ...
      stim_PIs [4] = 1'b1;      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
    ...
    begin 
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
        ...
        if (( part_POs [3] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          ...
          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sequential", name_POs [3], PATTERN, SCANCYCLE, part_POs [3] ); 
          end  
        end  
        else  begin 
      ...
      case ( CMD ) 
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "sequential_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
